...
@section('content')
    <div>
        <h2 class="text-2xl font-heading font-bold text-slate-900 dark:text-slate-200 text-center mb-6">Create account</h2>
        ...
        <form method="POST" action="{{ route('register.post') }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="mt-1 block w-full rounded-md border border-slate-700 bg-slate-800 text-slate-200 px-3 py-2" />
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="password" name="password_confirmation" required class="mt-1 block w-full rounded-md border border-slate-700 bg-slate-800 text-slate-200 px-3 py-2" />
            </div>
            <div class="flex items-center justify-between">
                <label class="inline-flex items-center gap-3 text-sm text-slate-300">
                    <input type="checkbox" name="agree" required class="h-4 w-4 rounded border-slate-700 bg-slate-800" />
                    <span>I agree to the <a href="#" class="text-indigo-400">Terms & Conditions</a></span>
                </label>
                <a href="{{ route('login') }}" class="text-sm text-indigo-400">Already have an account?</a>
                <button type="submit" class="inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-white">Create</button>
            </div>
        </form>
    </div>

    <div class="mt-4">
        <div class="text-center text-sm text-slate-400 mb-3">Or sign up with</div>
